<script lang="ts">
	...
	const baseClasses = `inline-flex items-center justify-center font-medium rounded-lg transition-colors ${variant !== 'text' ? 'focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500' : ''}`;
	...
	const spinnerColor = variant === 'primary' ? 'text-white' : 'text-indigo-600';
